...
PvPGame::PvPGame(std::vector<Attacker *> player1_attackers,
  ...
  ranges::for_each(
      ...
      [&, index = 0](const Attacker *player1_attacker) mutable {
        this->player1_attacker_id_to_index[player1_attacker->get_id()] =
            index++;
      });
  ranges::for_each(
      this->_player2_attackers,
      [&, index = 0](const Attacker *player2_attacker) mutable {
        this->player2_attacker_id_to_index[player2_attacker->get_id()] =
            index++;
      });
}

// TODO:- Accept player2_set_targets
PvPGame PvPGame::simulate(
  ...
  std::vector<Attacker *> player1_attackers(
      prev_state_player1_attackers.begin(), prev_state_player1_attackers.end());
  std::vector<Attacker *> player2_attackers(
      prev_state_player2_attackers.begin(), prev_state_player2_attackers.end());
  ...
  // Have separate player1_set_target and player2_set_target functions and set
  // it accordingly
  ranges::for_each(
      player_set_targets,
  ...
  ranges::for_each(spawn_positions_player1, [&](const auto &spawn_details) {
    ...
    if (Position::is_valid_pvp_spawn_position(
      ...
      if (!is_aerial) {
        player1_attackers.push_back(
            GroundAttacker::construct(attacker_type, position));
      } else {
        player1_attackers.push_back(
            AerialAttacker::construct(attacker_type, position));
      }
    }
  });
