...
void pvpGame(int p1_in, int p1_out, int p2_in, int p2_out) {
  ...
  for (size_t turn = 0; turn < turns; ++turn) {
    ...
    game = game.simulate(player1_set_targets, player1_spawn_positions,
    ...
    player1_input.put() << game.get_coins() << "\n";
    player2_input.put() << game.get_coins()
    ...
    Logger::log_coins(game.get_coins());
    Logger::log_coins(
        game.get_coins()); // separate coins for player1 and player 2
  }
  Logger::log_end();
...
int main(int argc, char *argv[]) {
  ...
  case GameType::PvP:
    ...
    // ..
    pvpGame(p1_in, p1_out, p2_in, p2_out);
    break;
  }
}